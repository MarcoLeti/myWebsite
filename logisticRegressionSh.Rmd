...
```{r}
data <- read.csv("data/ex2data2.txt", header = F)
...
evaluate <- function(lambda) {
    ...
    theta_optim <- optim(par = initial_theta, fn = costFunction, gr = gradient, 
...
lambdaVal <- seq(0, 100, 1)
results <- list()
for (i in 1:length(lambdaVal)) {
    lambda <- lambdaVal[i]
    results[[i]] <- evaluate(lambda)
}
```
...
inputPanel(
  ...
  sliderInput("bw_adjust", label = "Bandwidth adjustment:",
              min = 0, max = 100, value = 1, step = 1, width = '500px')
)

renderPlot({
lambda <- input$bw_adjust
